...
task Package -depends Build {
  foreach ($build in $builds)
  {
    ...
    robocopy "$sourceDir\Tonesoft.Settings\bin\Release\$finalDir" $workingDir\Package\Bin\$finalDir *.dll *.pdb *.xml /NFL /NDL /NJS /NC /NS /NP /XO /XF *.CodeAnalysisLog.xml | Out-Default
  }
